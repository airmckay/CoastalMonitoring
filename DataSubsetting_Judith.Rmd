...
dx <- input %>% mutate(filename = OUT.FILE.FS) %>% dplyr::select(filename, OUTDIR, FOLDER)
...
pt5$filepath <- paste0(pt5$OUTDIR, "/", pt5$FOLDER, "/", pt5$filename)

head(pt5$filepath)
pt5$fullpath1 <- gsub("\\\\", "/", pt5$filepath)
pt5$fullpath2 <- gsub("C:/TemporaryDatastorage", "P:/SW_CoastalMonitoring/Data_collection_2024", pt5$fullpath1)
pt5$fullpath3 <- gsub("D:/", "P:/SW_CoastalMonitoring/Data_collection_2024/", pt5$fullpath2)
...
pt6 <- pt5 %>% select(filename, fullpath, 
                      DATE, TIME, HOUR, 
                      DATE.12, TIME.12, HOUR.12, 
                      autoid, site, month) 
